...
class broot_select(Command):
    def execute(self):
        ...
        def show_error(msg):
            show_error_in_console(msg, fm)

        if "broot" not in get_executables():
            show_error("Could not find broot")
            return
...
if "fasd" in get_executables():
    hook_init_orig = ranger.api.hook_init

    def fasd_add(path):
        try:
            subprocess.check_output(['fasd', '--add', path])
        except subprocess.CalledProcessError:
            logger.error("failed to run fasd --add", exc_info=1)

    def hook_init(fm):
        def on_execute():
            for fobj in fm.thistab.get_selection():
                fasd_add(fobj.path)
        fm.signal_bind('execute.before', on_execute)

        def on_cd():
            fasd_add(fm.thistab.thisdir.path)
        fm.signal_bind('cd', on_cd)

        return hook_init_orig(fm)

    ranger.api.hook_init = hook_init

    class fasd_navigate(Command):
        def execute(self):
            run_and_cd("fasd -lR | fzf 2> /dev/tty", self.fm)

else:
    logger.warning("cannot find fasd program, the integration is disabled")
...
def main():
    if False:
        print(compose_hostname_list())

    if False:
        hostname = "ilya-thinkpad"
        #mount_path = "./ilya-thinkpad"
        mount_path = hostname2mount_path(hostname)
        cmd = ["sshfs", f"{hostname}:/", mount_path]
